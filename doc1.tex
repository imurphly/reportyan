...
\end{document}
\paragraph{title:  author: year:}
提出了一种基于区间分析的三次样条关节空间规划中机器人整体最小颠簸(MJ)轨迹的求解方法。
MJ轨迹是可取的，因为它们与人类关节运动的相似性，以及它们在路径跟踪和限制机器人振动方面的舒适性。
这使得它们成为机器人应用的有吸引力的选择，尽管没有考虑到机械臂动力学。
三次样条用于保证机器人运动中速度和加速度的整体连续性的框架中。
所得到的MJ轨迹规划是一个全局约束极大极小优化问题。\cite{824136}
提出了一种基于区间分析的新算法，并给出了其收敛性的确定性证明。
将所提出的规划方法应用于六关节机械手的实例，并与另一种MJ规划器进行了比较。
